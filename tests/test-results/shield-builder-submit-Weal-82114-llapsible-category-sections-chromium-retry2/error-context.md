...
- Name: shield-builder-submit.spec.ts >> WealthSpot Shield — builder submit flow >> Shield step renders 7 collapsible category sections
- Location: tests\web\e2e\shield-builder-submit.spec.ts:15:3
...
Locator: getByText('Builder Assessment').first()
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for getByText('Builder Assessment').first()
